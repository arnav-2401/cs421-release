...
cstackThen (("else", kelse):("if", kif):(c, kold):cstack) = Just ((c, transIfElse kif kelse . kold) : cstack)
cstackThen (("if", kif):(c, kold):cstack) = Just ((c, transIf kif . kold) : cstack)
...
transUntilLoop kloop state =
    case kloop state of
        (i:is, d, o)
            | i /= 0    -> (is, d, o)
            | otherwise -> transUntilLoop kloop (is, d, o)
        _ -> underflow

cstackUntil :: CStack -> Maybe CStack
cstackUntil (("begin", kloop):(c, kold):cstack) = Just ((c, transUntilLoop kloop . kold) : cstack)
